...
/**
 * @typedef {Object} BestSellersCardRefs
...
/** @extends {Component<BestSellersCardRefs>} */
class BestSellersCard extends Component {
  ...
  handleVariantSelect(event) {
    ...
    const allButtons = this.querySelectorAll('.bs-card__size-btn');

    for (const btn of allButtons) {
      btn.classList.remove('bs-card__size-btn--selected');
      btn.setAttribute('aria-pressed', 'false');
    }

    button.classList.add('bs-card__size-btn--selected');
  ...
  async handleAddToCart(event) {
    ...
    try {
      ...
      console.error('Best sellers add to cart error:', error);
    } finally {
      if (addButton) {
...
if (!customElements.get('best-sellers-card')) {
  customElements.define('best-sellers-card', BestSellersCard);
}

/**
 * @typedef {Object} BestSellersCarouselRefs
...
/** @extends {Component<BestSellersCarouselRefs>} */
class BestSellersCarousel extends Component {
  connectedCallback() {
    super.connectedCallback();
  ...
  handlePrev() {
    ...
    const cardWidth = track.querySelector('.bs-card')?.offsetWidth || 237;
